...
(SELECT s.site
      ...
      ,MAX(DATETIME(CONCAT(dt3.DATE, ' ', t3.TIME))) AS finish_bd_rfu
      ,MIN(DATETIME(CONCAT(dt1.DATE, ' ', t1.TIME))) AS start_working_first
      ,MAX(DATETIME(CONCAT(dt2.DATE, ' ', t2.TIME))) AS finish_working_last
...
SELECT grouped.*
      ...
      ,IF(grouped.finish_working_last < grouped.start_working_first,
      SAFE_CAST(DATE_DIFF(DATETIME(grouped.finish_working_last) + interval 1 day, DATETIME(grouped.start_working_first), SECOND)/3600 AS FLOAT64),
      SAFE_CAST(DATE_DIFF(DATETIME(grouped.finish_working_last), DATETIME(grouped.start_working_first), SECOND)/3600 AS FLOAT64)) AS AGING_ACTIVITY_HOUR
FROM grouped